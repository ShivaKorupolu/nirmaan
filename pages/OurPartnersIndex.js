...
function OurPartnersIndex() {
  ...
  return (
    <>
      <div className="bg-nirmaan-secondary w-full lg:h-80 sm:h-96 border border-b-2 border-b-blue-500 border-r-2 border-r-blue-500  sm:justify-center sm:items-center sm:text-center  lg:px-20 lg:pb-6 lg:pt-4 text-left">
      <div className=" sm:mb-1 lg:mb-4 flex justify-between items-center">
            <h1 className="lg:text-3xl text-nirmaan-darker uppercase md:text-2xl text-xl font-bold inter mb-7 lg:mt-7 underline underline-offset-8">
        ...
        <div className=" lg:pt-4 sm:pt-4 md:pt-4 sm:text-center lg:text-center grid lg:grid-cols-6 grid-cols-2 p-2 mt-4 text-ellipsis md:h-auto md:pb-4 md:grid-cols-3 lg:gap-y-4 md:gap-y-2 gap-3 ">
          {logos
            ? logos.map((show, logo_title) => logo_title > 5 ||(
                <div
                  key={show.id}
                  className="bg-blue-50 h-28 w-44 border rounded-lg p-2 shadow-lg flex justify-center items-center text-center "
                >
                  {show.image ? (
                    <img
                      src={show.image}
